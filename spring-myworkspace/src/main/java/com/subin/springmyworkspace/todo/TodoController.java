...
public class TodoController {
	...
	// DI(의존성주입, Dependency Injection): 객체를 사용하는 곳이 아닌 외부에서 객체를 생성하여 주입하여 주는 것

	// TodoController 객체를 생성하는 시점에 TodoRepository 인터페이스에 맞는 객체를 생성 후 주입
	// TodoController 인스턴스는 Spring IoC 컨테이너에서 생성함

	// TodoRepository 인터페이스에 맞는 SQL문이 실행가능한 객체를 생성함
	...
	// 1건 추가
	// POST /todos {memo:<h1>"Spring공부하기"<h1>도 가능}
	...
	// 1건 조회
	// Get /todos/1 -> todo목록에서 id가 1인 레코드
	...
	public Todo getTodo(@PathVariable int id, HttpServletResponse res) {
		...
		return todo.get();
		// return todo.orElse(null); // id가 있으면 데이터나오고 없으면 널 값.
	}
	...
	public boolean removeTodo(@PathVariable int id, HttpServletResponse res) {
		...
		// soft-delete: 특정 컬럼을 업데이트하여 조회할 때 안보이게함
		// hard-delete: 실제로 DELELTE문으로 레코드를 삭제
		// JPA repository는 기본적으로 hard-delete 사용
		repo.deleteById(id);
		return true;
	}

	// 1건 수정
	// PUT/ todos/1 {"memo":"JPA 공부하기"}
	@PutMapping(value = "/todos/{id}")
	public Todo modifyTodo(@PathVariable int id, @RequestBody Todo todo, HttpServletResponse res) {
		// 특정 필드만 업데이트 해야함
		// 이전에 입력한 시스템 필드는 변경하면 안됨
